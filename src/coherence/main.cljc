(ns coherence.main
  "Kotodama WASM entrypoint for the Coherence clean-room actor (L4) — Clojure port.
  ...
  facts (`coherence.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "coherence")
(def tier "L4")
...
(def entity-specs
  [{:entity "Player"      :plural "players"      :id-prefix "coherenc_pla"
    :fields [:handle :level :xp :region]         :required [:handle :level]
    :coerce {:level :int :xp :int}               :refs {}}
   {:entity "Session"     :plural "sessions"     :id-prefix "coherenc_ses"
    :fields [:playerId :platform :startedAt :durationSec] :required [:platform :startedAt]
    :coerce {:durationSec :int}                  :refs {:playerId "Player"}}
   {:entity "Match"       :plural "matches"      :id-prefix "coherenc_mat"
    :fields [:mode :status :playerCount]         :required [:mode :status]
    :coerce {:playerCount :int}                  :refs {}}
   {:entity "Leaderboard" :plural "leaderboards" :id-prefix "coherenc_lea"
    :fields [:name :metric :season]              :required [:name :metric]
    :coerce {}                                   :refs {}}
   {:entity "Inventory"   :plural "inventories"  :id-prefix "coherenc_inv"
    :fields [:playerId :itemId :quantity]        :required [:quantity]
    :coerce {:quantity :int}                     :refs {:playerId "Player"}}
   {:entity "Achievement" :plural "achievements" :id-prefix "coherenc_ach"
    :fields [:playerId :code :unlockedAt]        :required [:code :unlockedAt]
    :coerce {}                                   :refs {:playerId "Player"}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"coherence.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "coherence-compat" :tier tier :entities entities} 200])
...
(defn start! [] :coherence-compat/ready)
